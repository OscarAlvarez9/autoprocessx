...
const services: Array<{
    ...
    },
    {
        id: "02",
        serviceKey: "ai-apps",
        title: "Tu conocimiento, consultable al instante",
        alias: "Aplicaciones IA Corporativas",
        desc: "Asistentes que conocen vuestros procedimientos mejor que cualquier empleado nuevo. Sistemas RAG anclados a vuestro capital intelectual que citan la fuente de cada respuesta.",
    ...
    },
    {
        id: "03",
        serviceKey: "chatbot",
        title: "Atención y ventas sin intervención",
...
export default function Services() {
    return (
        <section id="servicios" className="py-24 md:py-32 text-[#09090B] overflow-hidden relative">
                ...
                <div className="flex flex-col lg:flex-row items-end justify-between gap-10 mb-16 md:mb-24">
                    <div className="max-w-3xl">
                        <div className="flex items-center gap-4 mb-8">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500">Deployment Capabilities</span>
                        </div>
                        <h2 className="text-3xl md:text-4xl font-black tracking-tight leading-[1.05] mb-5">
                            Nuestras <span className="italic text-zinc-600">Verticales</span>.
                        </h2>
                        <p className="text-base md:text-lg text-zinc-600 font-medium leading-relaxed max-w-2xl">
                    ...
                    {/* Color legend */}
                    <div className="hidden lg:flex flex-col gap-2 px-5 py-4 rounded-2xl bg-[#FFFFFF] border border-[#E4E4E7] backdrop-blur-xl">
                        {(Object.keys(serviceColors) as ServiceKey[]).map((k) => {
                            const c = serviceColors[k]
                            return (
                                <div key={k} className="flex items-center gap-3">
                                    <div className={`h-2 w-2 rounded-full ${c.dot}`} />
                                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-600">{c.name}</span>
                                </div>
                            )
                        })}
                    </div>
                    ...
                    {services.map((service, i) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={`group relative flex flex-col p-8 md:p-10 rounded-[32px] md:rounded-[40px] bg-[#FFFFFF] border border-[#E4E4E7] ${c.borderHover} transition-all duration-500 overflow-hidden`}
                            >
                                ...
                                <div className="relative mb-8">
                                    ...
                                    <h3 className="text-2xl md:text-3xl font-black mb-5 tracking-tight leading-tight text-[#09090B]">
                                        {service.alias}
                                    </h3>
                                ...
                                <div className="relative mb-8 space-y-2.5">
                                    {service.features.map((feature, idx) => (
                                        <div key={idx} className="flex items-center gap-3">
                                            ...
                                            <span className="text-[11px] font-bold text-zinc-600 tracking-wide">
                                                {feature}
                                            </span>
                                        </div>
                                    ))}
                                ...
                                <div className="relative mt-auto pt-6 border-t border-[#E4E4E7]">
                                    <div className="flex flex-wrap gap-1.5 mb-6">
                                        {service.tech.map((t, idx) => (
                                            <span
                                                key={idx}
                                                className={`px-2.5 py-1 rounded-md bg-[#FFFFFF] border border-[#E4E4E7] text-[10px] font-black uppercase tracking-wider text-zinc-600 whitespace-nowrap`}
                                            >
                                                {t}
                                            </span>
                                        ))}
                                    ...
                                    <Link href={service.href}>
                                        <Button
                                            className={`w-full bg-zinc-100 border border-[#E4E4E7] text-[#09090B] ${c.hoverBg} hover:text-black ${c.hoverBorder} h-14 rounded-xl font-black uppercase tracking-[0.25em] text-[10px] transition-all active:scale-95`}
                                        >
                                            Ver servicio
